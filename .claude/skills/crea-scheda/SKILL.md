...
| **#044** | **Cerqua** — evoluzione di Totera. RISERVATO: scheda ancora da scrivere, non riassegnare. |
...
Regola: scansiona i file in `Wikascolemon/` e `schede _Pokemon/`, e assegna il **primo numero libero coerente col concept** (una linea starter va nel suo blocco riservato; una specie qualsiasi prende il primo libero da #054 in su (il #044 è riservato a Cerqua)). Non assegnare mai "il più alto + 1" senza guardare la tabella. La navigazione `.dexnav` tollera i buchi: il vicino mancante si scrive `<span class="dim">#0NN: ???</span>`.
