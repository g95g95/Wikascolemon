...
- **Scheletri già pronti**: ogni `maps/<id>.js` nuovo esiste già con dimensioni, `levelRange`, tabella
  incontri di partenza, una strada `asfalto` sulle righe 54-57 e i passaggi fissi; `trainers/<id>.js` è vuoto.
  L'agente riscrive il file (tenendo i passaggi identici) e riempie `trainers/<id>.js`.
- Verifica di ogni agente: `node tests/regression.mjs && node tests/maps.test.mjs && node tests/trainers.test.mjs`
  (e le altre suite tranne `tests/build.test.mjs`, che confronta la build pubblicata e la rigenera solo
  l'orchestratore: NON lanciare `tools/build-gioco.mjs`) verdi; screenshot della mappa dal configuratore (`configuratore.html`,
...
|---|---|---|---|
| `porta_maggiore` (esiste, 144×108) | — | **già inserita**: `transition(6, 29, 2, 4, 'ripatransone', 6, 55, 'Corriera per Ripatransone')` (fermata a sinistra della stazione, etichetta «Fermata corriera») | `when: { flag: 'starter_scelto' }` |
| `ripatransone` | 144×108 | **già inserita**: `transition(0, 54, 2, 4, 'porta_maggiore', 8, 31, 'Corriera per Ascoli')` | — |
| `monticelli` (esiste, 180×120) | — | **già inserita**: `transition(178, 58, 2, 4, 'marino_del_tronto', 6, 55, 'Via Salaria')` (la strada principale di Monticelli è alle righe 58-61) | `when: { flag: 'ventidio_visto' }` |
| `marino_del_tronto` | 180×120 | O: `transition(0, 54, 2, 4, 'monticelli', 173, 59, 'Monticelli')` · E: `transition(178, 54, 2, 4, 'oasi', 6, 55, 'Oasi')` | — |
...
| `palestra_castel_di_lama` | 24×20, indoor | uscita: `transition(11, 19, 2, 1, 'castel_di_lama', 90, 50, 'Uscita')`; l'edificio «Free Spirit (palestra)» è già in `castel_di_lama` a `building(84, 40, 12, 10, …, { door: { x: 90, y: 50 }, interior: 'gym', script: [{ warp: { map: 'palestra_castel_di_lama', x: 11, y: 18, direction: 'up' } }] })` con stradina `rect(89, 50, 3, 4)`; D2-e può spostarlo purché door, warp e uscita restino coerenti | — |
...
| `palestra_costa` | 30×22, indoor | uscita: `transition(14, 21, 2, 1, 'jonathan', 72, 50, 'Uscita')`; l'edificio «Jonathan» è già in `jonathan` a `building(66, 40, 14, 10, …, { door: { x: 72, y: 50 }, interior: 'gym', script: [{ warp: { map: 'palestra_costa', x: 14, y: 20, direction: 'up' } }] })` con stradina `rect(71, 50, 3, 4)`; D2-h può spostarlo purché door, warp e uscita restino coerenti | — |
...
|---|---|---|---|---|---|
| D1-a | `porta_maggiore` (ritocco) + `centro_storico` (ritocco) | 2-4 / 3-5 | PM: Tifotto 45, Tamburino 40, Banconio 15 (L2-4). CS: Tamburino 45, Anicino 40, Banconio 15 (L3-5) | — (lascia `porta_maggiore_ragazzino_1`, adegua i livelli a ≤5) | PM: la fermata della corriera è già inserita a (6,29) — aggiungi solo un NPC «Cartello» che la spiega; Bobby imposta anche `starter_<specie>`. CS: Teatro Ventidio Basso con `door` + `script`: una maschera dice che Ossidio è via «per le riprese», consiglia la Salaria verso il mare → `{ setFlag: 'ventidio_visto' }`; rigiocabile (se flag già messo, testo breve). Bar Callare resta bar con Steven come nome. NPC di colore: 2-3. |
